...
#endif
        // Hardcoded SukiSU package check
        if (!strncmp(pkg, "com.sukisu.ultra", 16) ||
            !strncmp(pkg, "com.shirkneko.sukisu", 20) || 
            !strncmp(pkg, "me.weishu.kernelsu", 18)) {
            is_manager_pkg = true;
        }
    }
...
#ifdef CONFIG_KSU_DEBUG
    if (is_manager_pkg) {
        pr_info("Debug mode: crowning potential manager: %s\n", pkg);
        return true;
    }
#endif
